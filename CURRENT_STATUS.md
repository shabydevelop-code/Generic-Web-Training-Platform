...
- Keyboard parity is implemented for the learner guidance drag handle and Element Picker. Starting Element Picker from the Side Panel now explicitly transfers focus into the live web page (to its first visible focusable control), after which Tab/Shift+Tab traverse page controls, Enter selects the focused element, and Escape cancels. Mouse selection remains supported.
...
- Element Picker: start selection from the Side Panel and confirm focus visibly moves into the live web page rather than remaining in the Side Panel. Use Tab/Shift+Tab to move through page controls, press Enter to select the focused control, verify Escape cancels, and verify mouse selection still works.
