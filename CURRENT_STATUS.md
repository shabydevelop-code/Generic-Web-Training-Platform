...
- **Actionable-step auto advance (2026-09-22; pending local verification):** added a backward-compatible `GuideSteps.InteractionType` migration with default `auto` (existing rows are upgraded in place; no DB replacement). Existing and new `auto` steps dynamically recognize actionable targets (`button`, links, button-like inputs, `role=button/link`) at render time. Actionable steps without field validation no longer show Next/Finish; activating the highlighted target advances/completes the guide. Non-actionable and validation steps retain manual navigation. API/editor persistence carries `interactionType`, and the schema default is `auto`.
