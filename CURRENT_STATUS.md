...
- **Restricted-page regression test correction (2026-09-22; pending rerun):** the new Playwright check reached the expected localized unsupported-page UI successfully, but its Hebrew assertion used wording different from the actual translation (`דפדפן פנימיים` vs. `פנימיים של הדפדפן`). Corrected the test assertion only; runtime behavior was already correct. Rerun the targeted `browser-internal page` test before the full suite.
...
- **Editor Element Picker cancellation (2026-09-22; pending local verification):** active picker mode is now explicit and cancellable from the Side Panel. After starting selection, the button changes to localized `Cancel element selection / בטל בחירת אלמנט`; pressing it sends `GWTP_CANCEL_ELEMENT_PICKER` to all reachable frames, removes picker listeners/hover, restores the normal Select button, and shows the localized cancellation status beside Element Selection. Existing Escape cancellation now also resets the Side Panel button/status. Closing or switching the Step editor cancels active picker mode automatically. Added a Stage 6 Playwright regression for Side Panel cancellation and hover cleanup.
